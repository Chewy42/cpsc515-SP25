...
def collisionTest_spheres(center1, radius1, center2, radius2):
    ...
    for i in range(3):
        distance_squared += (center1[i] - center2[i]) ** 2
...
def collisionTest_sphereAABB(center1, radius1, min_coord2, max_coords2):
    ...
    for i in range(3):
        closest_point[i] = max(min_coord2[i], min(center1[i], max_coords2[i]))

    # calculate the squared distance between closest point to the center
    distance_squared = sum((closest_point[i] - center1[i]) ** 2 for i in range(3))

    # compare squared distance with squared radius  
    radius_squared = radius1 ** 2
...
def main():
    ...
    rx, ry = (0,0)
    tx, ty = (0,0)
    zpos = 18  # Pull camera further back for third-person view
    rotate = False
    move = False
    ...
    while True:
        ...
        for e in pygame.event.get():
            if e.type == QUIT:
                ...
                laplacian_smooth(obj2, iterations=1)
                compute_vertex_normals(obj2)
                obj2.rebuild_gl_list()
        ...
        # (Optional: Implement player movement here if needed)

        # collision detection: check against all panthers
        min_coords1, max_coords1, center1, radius1 = obj1.cal_minMax()
        bCollide = False
        for panther in panthers:
            min_coords2, max_coords2, center2, radius2 = panther['obj'].cal_minMax()
            if BV1_type == panther['BV_type']:
                if BV1_type == "sphere":
                    if collisionTest_spheres(center1, radius1, center2, radius2):
                        bCollide = True
                        break
                elif BV1_type == "AABB":
                    if collisionTest_AABBs(min_coords1, max_coords1, min_coords2, max_coords2):
                        bCollide = True
                        break
            else:
                if BV1_type == "sphere" and panther['BV_type'] == "AABB":
                    if collisionTest_sphereAABB(center1, radius1, min_coords2, max_coords2):
                        bCollide = True
                        break
                elif panther['BV_type'] == "sphere" and BV1_type == "AABB":
                    if collisionTest_sphereAABB(center2, radius2, min_coords1, max_coords1):
                        bCollide = True
                        break
        ...
        # draw all panthers
        for panther in panthers:
            draw_mesh(obj=panther['obj'], bv_type=panther['BV_type'])
        glPopMatrix()
        drawAxes()

        # Win condition check: if player (Suzanne) crosses finish_z
        # We'll use the z value of the center of obj1 (Suzanne)
        _, _, center1, _ = obj1.cal_minMax()
        if not win and center1[2] >= finish_z:
            win = True
            print("You Win!")
        if win:
            # Overlay win message
            ...
            display_surface.blit(text, (width//2 - text.get_width()//2, height//2 - text.get_height()//2))
            pygame.display.flip()
