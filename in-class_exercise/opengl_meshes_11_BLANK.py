...
def laplacian_smoothing_avg():
    ...
    for edge in edges:
        if 3 in edge:
            neighbors.append(edge[0] if edge[0] != 3 else edge[1])

    # calculating the average position of the neighbors
    avg_neighbor_position = np.mean(vertices[neighbors], axis=0)

    # assign the average position to v3 (index 3)
    vertices[3] = avg_neighbor_position
...
def laplacian_smoothing(lambda_val = 0.1):
    ...
    vertices[3] = vertices[3] + lambda_val * (np.mean(vertices[neighbors], axis=0) - vertices[3])
